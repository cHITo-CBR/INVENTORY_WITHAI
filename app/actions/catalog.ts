...
export async function getArchivedCategories(): Promise<CategoryRow[]> {
  const { data, error } = await supabase.from("product_categories").select("*").eq("is_archived", true).order("name");
  if (error || !data) return [];
  return data;
}
...
export async function createCategory(formData: FormData) {
  ...
  const { error } = await supabase.from("product_categories").insert([{ name, description: description || null }]);
...
export async function updateCategory(id: number, formData: FormData) {
  ...
  const description = formData.get("description") as string;
  if (!name) return { error: "Name is required." };
  const { error } = await supabase.from("product_categories").update({ name, description: description || null }).eq("id", id);
  if (error) return { error: error.message };
  revalidatePath("/catalog/categories");
  return { success: true };
}
...
export async function archiveCategory(id: number) {
  const { error } = await supabase.from("product_categories").update({ is_archived: true }).eq("id", id);
  if (error) return { error: error.message };
  revalidatePath("/catalog/categories");
  return { success: true };
}
...
export async function getBrands(): Promise<BrandRow[]> {
  const { data, error } = await supabase.from("brands").select("*").eq("is_archived", false).order("name");
  if (error || !data) return [];
  return data;
}

export async function getArchivedBrands(): Promise<BrandRow[]> {
  const { data, error } = await supabase.from("brands").select("*").eq("is_archived", true).order("name");
  if (error || !data) return [];
  return data;
}
...
export async function createBrand(formData: FormData) {
  ...
  const description = formData.get("description") as string;
  if (!name) return { error: "Name is required." };
  const { error } = await supabase.from("brands").insert([{ name, description: description || null }]);
  if (error) return { error: error.message };
...
export async function updateBrand(id: number, formData: FormData) {
  ...
  const description = formData.get("description") as string;
  if (!name) return { error: "Name is required." };
  const { error } = await supabase.from("brands").update({ name, description: description || null }).eq("id", id);
  if (error) return { error: error.message };
  revalidatePath("/catalog/brands");
  return { success: true };
}
...
export async function archiveBrand(id: number) {
  const { error } = await supabase.from("brands").update({ is_archived: true }).eq("id", id);
  if (error) return { error: error.message };
  revalidatePath("/catalog/brands");
  return { success: true };
}
...
export async function getUnits(): Promise<UnitRow[]> {
  const { data, error } = await supabase.from("units").select("*").eq("is_archived", false).order("name");
  if (error || !data) return [];
  return data;
}

export async function getArchivedUnits(): Promise<UnitRow[]> {
  const { data, error } = await supabase.from("units").select("*").eq("is_archived", true).order("name");
  if (error || !data) return [];
  return data;
}
...
export async function createUnit(formData: FormData) {
  ...
  if (!name) return { error: "Name is required." };
  const { error } = await supabase.from("units").insert([{ name, abbreviation: abbreviation || null }]);
  if (error) return { error: error.message };
...
export async function getPackagingTypes(): Promise<PackagingRow[]> {
  const { data, error } = await supabase.from("packaging_types").select("*").eq("is_archived", false).order("name");
  if (error || !data) return [];
  return data;
}

export async function getArchivedPackagingTypes(): Promise<PackagingRow[]> {
  const { data, error } = await supabase.from("packaging_types").select("*").eq("is_archived", true).order("name");
  if (error || !data) return [];
  return data;
}
...
export async function createPackagingType(formData: FormData) {
  ...
  const description = formData.get("description") as string;
  if (!name) return { error: "Name is required." };
  const { error } = await supabase.from("packaging_types").insert([{ name, description: description || null }]);
  if (error) return { error: error.message };
...
export async function updatePackagingType(id: number, formData: FormData) {
  ...
  const description = formData.get("description") as string;
  if (!name) return { error: "Name is required." };
  const { error } = await supabase.from("packaging_types").update({ name, description: description || null }).eq("id", id);
  if (error) return { error: error.message };
  revalidatePath("/catalog/packaging");
  return { success: true };
}
...
export async function archivePackagingType(id: number) {
  const { error } = await supabase.from("packaging_types").update({ is_archived: true }).eq("id", id);
  if (error) return { error: error.message };
  revalidatePath("/catalog/packaging");
  return { success: true };
}
