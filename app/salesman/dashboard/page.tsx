    ...
    <div className="space-y-6 max-w-2xl mx-auto">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#005914] via-[#007a1f] to-emerald-500 p-6 text-white shadow-2xl shadow-green-900/30">
        ...
        <div className="relative z-10">
          <p className="text-green-100 text-xs font-semibold uppercase tracking-widest mb-1">{greeting()}</p>
          <h2 className="text-2xl font-black tracking-tight">{user?.full_name ?? "Field Partner"}</h2>
          <p className="text-green-200 text-sm mt-1 flex items-center gap-1.5">
            <Calendar className="w-3.5 h-3.5" />
            {new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}
          </p>
        </div>
        <div className="relative z-10 mt-5 flex items-center gap-3 bg-white/10 backdrop-blur-sm rounded-2xl p-3">
          <Target className="w-5 h-5 text-green-200" />
          <div className="flex-1">
            <div className="flex items-center justify-between">
              <p className="text-[10px] font-bold text-green-100 uppercase tracking-wider">Daily Sales Target</p>
              <p className="text-xs font-bold text-green-200">60%</p>
            </div>
            <div className="w-full h-2 bg-green-900/40 rounded-full mt-1.5 overflow-hidden">
              <div className="h-full bg-gradient-to-r from-green-300 to-emerald-200 rounded-full w-[60%] transition-all duration-1000" />
            </div>
          </div>
        </div>
      </div>
